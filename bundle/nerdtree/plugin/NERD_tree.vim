...
" Add by Liang Feng <fliang98 AT gmail DOT com>
call s:initVariable("g:NERDTreeFollowOpenInNewTab", 0)
call s:initVariable("g:NERDTreeExitOnlyWindow", 0)
" End of add
...
augroup NERDTree
    ...
    " Add by Liang Feng <fliang98 AT gmail DOT com>
    if g:NERDTreeExitOnlyWindow == 1
        autocmd BufEnter NERD_tree_* call s:exitOnlyWindow()
    endif
    " End of add
augroup END
...
function! s:Bookmark.openInNewTab(options)
    ...
    if self.path.isDirectory
        ...
        " Modified by Liang Feng <fliang98 AT gmail DOT com>
        if bufloaded(substitute(self.path.str({'format': 'Edit'}), '\\', '/', 'g'))
            exec "sbuffer " . self.path.str({'format': 'Edit'})
        else
        exec "tabedit " . self.path.str({'format': 'Edit'})
    endif

        if g:NERDTreeFollowOpenInNewTab == 1
            call s:initNerdTree('')
            wincmd p
        endif
        " End of modification
...
"============================================================
" Add by Liang Feng <fliang98 AT gmail DOT gmail>
"FUNCTION: s:exitOnlyWindow {{{2
"When switch buffer, exit vim if only the NERDTree window is present.
"If there is more than one tab present, close current tab where the NERDTree
"window located.
function s:exitOnlyWindow()
    if winbufnr(2) == -1
        if tabpagenr('$') == 1
            " Only one tab present
            " use bdelete to make '0 command happy.
            " Use noautocmd to work with some other plugins smoothly,
            " such as projetmgr.
            noautocmd bdelete
            quit
        else
            " More than one tab present
            close
        endif
    endif
endfunction
